...
def balanced(m):
    ...
    if is_weight(m):
        ...
        b = [balanced(end(side)) for side in sides(m)]
        return all(b) and len(set([total_weight(end(side)) * length(side) for side in sides(m)])) == 1
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount,verify_password):
        ...
        if attempts==0:
                return "Your account is locked. Attempts: "+str(attempts_password)
        if password==verify_password:
            ...
            attempts-=1
            attempts_password.append(verify_password)
...
def make_joint(withdraw, old_password, new_password):
    ...
    if type(value)==str:
       return value
        ...
        def joint_withdraw(amount,verify_password):
